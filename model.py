...
class BaselineModel2(nn.Module):
    def __init__(self, embed_dim, hidden_dim, image_dim=2048, batch_size=8, vocab_size=1004, start_token=1, end_token=2, pad_token=0, unk_token=3, max_seq_len=16, pretrained_embeds=[], device="cuda"):
        ...
        get_features = copy.deepcopy(nn.Sequential(*list(model.children())[:-1]))
        del model
        ...
        self.hinit = nn.Linear(hidden_dim, hidden_dim)
        self.cinit = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, filenames, captions=[]):
        ...
        image_features = torch.cat([get_image(filename, self.scaler, self.totensor, self.normalize) for filename in filenames], dim=0).to(self.device) # Bx3x224x224 images
        image_features = self.get_features(image_features) # Bx2048x1x1

        image_features = image_features.reshape(B, -1)
        image_features = self.linear(image_features) # BxH
        ...
        if len(captions) > 0: # In Training
            ...
            x = torch.zeros(B,1, dtype=torch.long, device=self.device) + self.start_token

            out = torch.zeros(B,self.max_seq_len, dtype=torch.long, device=self.device)
...
class TransformerModel2(nn.Module):
    ...
    def forward(self, filenames, captions=[], orders=[], generate=1):
        ...
        image_features = self.linear(image_features.permute(0,2,3,1)) # Bx7x7xH or Bx4x4xH
